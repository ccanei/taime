...
//
// Função extraída de app/api/cron/newsletter-send/route.ts (Camada 4, v2 do
// encadeamento). Antes era um cron próprio agendado 11:30 UTC; o plano Hobby
// da Vercel garante precisão per-hour (±59 min), o que tornava impossível
// encadear dois crons na mesma janela 11h UTC (briefing + envio). Agora o
// briefing chama esta função no fim do seu próprio cron, eliminando a corrida.
//
// Comportamento idêntico ao da rota original: lê o briefing do dia, lista
// assinantes ativos, envia em lote via Resend batch, grava newsletter_sends +
// newsletter_send_recipients com snapshot do conteúdo, e respeita os skips
// (sem briefing, sem ativos, idempotência por briefing_date + status).
//
// A rota app/api/cron/newsletter-send/route.ts continua existindo como
// gatilho manual (curl com Bearer CRON_SECRET) e chama esta mesma função.
