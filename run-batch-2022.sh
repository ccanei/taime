#!/bin/bash
# Runner autonomo do batch 2022 (Opus 4.8, NO_AUTO_PUBLISH=1).
# PASS 1: batch completo (le batch-periods.json = 24 periodos, mais novo primeiro).
# PASS 2 e 3: --resume automatico para reprocessar falhas transitorias (rede,
# rate limit, parse nao-deterministico do Opus). Periodos bloqueados por guarda de
# idempotencia (clusters ja existentes) apenas re-falham e ficam registrados; nada
# e apagado. Nao toca nos scripts do pipeline.
set -uo pipefail
cd "$(dirname "$0")"
...
echo "======== BATCH_2022_DONE $(date -u) ========" >> "$LOG"
