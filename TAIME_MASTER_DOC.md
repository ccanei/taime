...
### Camada 4: Newsletter do Radar (resumo SEMANAL sintetizado por tema)

Implementada. A captura de inscritos vive em `/radar` (componente `NewsletterSignup`). O e-mail agora e **SEMANAL**, enviado toda **segunda 09h BRT (12h UTC)**, com um resumo que **sintetiza os briefings da semana por tema** (nao repete o conteudo diario). O briefing diario e a coleta continuam rodando normalmente e alimentando o dashboard `/radar` e o acervo; so o e-mail mudou de cadencia.

**Por que semanal**: o conteudo diario ficava repetitivo no inbox. Uma sintese semanal por tema (3 a 5 temas dominantes, cada um com um paragrafo executivo, abrindo com "a semana em uma frase") entrega mais sinal e menos ruido.

**Layout editorial diagramado (2026-08)**: o e-mail semanal deixou de ser texto corrido. Agora tem: cabecalho "TAIME RADAR" + "SEMANA DE {seg} A {dom} {mes}" (intervalo EXATO da janela lida); titulo da edicao (serif); kicker "A SEMANA EM UMA FRASE" + lead; um bloco "OS SINAIS DA SEMANA" (grafico de composicao por tema com barras proporcionais e o tema dominante destacado); e cada tema com numero serif azul, corpo, chips opcionais (lugares/fatos contaveis citados) e pull quote opcional (max 2 por edicao). **REGRA INVIOLAVEL**: o grafico so exibe contagens REAIS de sinais por tema, atribuidas pelo modelo ao classificar os sinais da semana. O TOTAL vem dos `radar_briefings` (`signal_count`). Nenhum numero no e-mail sem base nos sinais: se as contagens nao vierem validas, o grafico e OMITIDO (degradacao segura), o e-mail sai sem ele. Template segue dark table-based (#0F172A), compativel Outlook, `escapeHtml` em campo dinamico.

**Fluxo real**:

1. **Briefing diario** via `/api/cron/radar-briefing` (11:00 UTC, 08:00 BRT). Gera 1 linha em `radar_briefings` com `title_pt/en` e `body_pt/en` a partir dos sinais das ultimas 24h. Idempotente por `briefing_date`. **Nao dispara mais e-mail** (o encadeamento com o envio foi removido): apenas coleta e grava o briefing, que segue alimentando o dashboard e o acervo.
2. **Envio SEMANAL** via `/api/cron/newsletter-weekly` (segunda 12:00 UTC). A logica vive em `lib/newsletter/send-weekly.ts` (`sendWeeklyNewsletter()`): le os briefings da **semana coberta** (segunda a domingo da semana ANTERIOR, ja que o envio e na segunda: janela `[currentMonday-7, currentMonday-1]`, filtrada por `briefing_date` gte/lte, batendo exatamente com o "SEMANA DE {seg} A {dom}" do cabecalho), deriva o TOTAL real de sinais somando `signal_count` dos briefings, sintetiza com Sonnet 4.6 num **JSON estruturado por idioma** (`{pt:{titulo,lead,temas:[{numero,titulo,corpo,pull_quote,chips,signal_count}],total_signals}, en:{...}}`; base estrita nos briefings, fontes por categoria, sem travessao, sem valores monetarios, PT e EN nativos), e delega envio + historico ao nucleo compartilhado `lib/newsletter/shared.ts` (`deliverNewsletter()`) passando um `buildHtml` custom (o layout editorial) e o snapshot `structured`. `deliverNewsletter` mantem inscritos ativos, e-mail por idioma, lotes via `/emails/batch` do Resend. Idempotencia POR SEMANA: `briefing_date` = a segunda-feira do envio. Skips: `no_briefings_this_week`, `already_sent`, `no_active_subscribers`. **Preview**: `?preview={email}&lang=pt|en` (`sendWeeklyPreview`) envia so para 1 endereco, sem historico e sem idempotencia, para conferir o layout antes do deploy; o caminho agendado (sem `preview`) e inalterado. A rota `/api/cron/newsletter-send` continua como gatilho **manual** do diario (`sendDailyNewsletter()`, sobre o mesmo `deliverNewsletter()` com o template classico), intacta.
...
4. **Histórico** próprio. Cada envio grava 1 linha em `newsletter_sends` com snapshot do conteúdo (subject + body PT e EN) e contagens (`recipient_count`, `sent_count`, `failed_count`, `status`). O semanal grava tambem a coluna `structured` (jsonb) com a estrutura editorial exata do que foi enviado (lead + temas + `signal_count` real + janela da semana); a coluna e opcional (`add-newsletter-structured.sql`) e o insert e resiliente: se ela nao existir, o e-mail sai igual e o snapshot grava sem o campo. Cada destinatário vira 1 linha em `newsletter_send_recipients` com `delivered` e `error`. O snapshot é deliberado: preserva o que foi enviado mesmo que o briefing seja editado depois.
