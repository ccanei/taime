...
2. **Envio SEMANAL** via `/api/cron/newsletter-weekly` (segunda 12:00 UTC). A logica vive em `lib/newsletter/send-weekly.ts` (`sendWeeklyNewsletter()`): le os briefings dos ultimos 7 dias (`radar_briefings?briefing_date=gte.{hoje-7d}`), sintetiza por tema com Sonnet 4.6 (base estrita nos briefings, fontes por categoria, sem travessao, sem valores monetarios, PT e EN nativos, JSON `{title_pt,title_en,body_pt,body_en}`), e delega envio + historico ao nucleo compartilhado `lib/newsletter/shared.ts` (`deliverNewsletter()`): inscritos ativos, e-mail por idioma (template dark table-based), lotes via `/emails/batch` do Resend. Idempotencia POR SEMANA: `briefing_date` = a segunda-feira do envio; skip `already_sent` se ja houve envio `sent`/`partial` nessa data. Skips: `no_briefings_this_week`, `already_sent`, `no_active_subscribers`. Como le briefings ja gravados dos 7 dias anteriores, nao depende do briefing de segunda nem ha corrida de horario. A rota `/api/cron/newsletter-send` continua como gatilho **manual** do diario (`sendDailyNewsletter()`, agora sobre o mesmo `deliverNewsletter()`), para reenvio pontual.
